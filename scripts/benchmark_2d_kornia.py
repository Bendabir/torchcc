...
BS = 4
OUTPUT = Path(f"benchmarks/2d-kornia-{BS}.csv")
K = 50_000
DEVICE = "cuda"
...
with tqdm(unit="image", total=len(paths)) as bar:
    for ps in _batchify(paths, n=BS):
        ...
        tensor = torch.from_numpy(batch).to(DEVICE).unsqueeze_(1).to(torch.float32)
        start = time.perf_counter()

        with torch.no_grad():
            _ = kornia.contrib.connected_components(tensor)
        ...
        results["implementation"].extend(["kornia"] * length)
